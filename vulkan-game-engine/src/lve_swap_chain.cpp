...
namespace lve {
  ...
  void LveSwapChain::createDepthResources() {
    ...
    // Memorizza il formato di profondità scelto per poter verificare la compatibilità della swap chain durante la ricreazione
  ...
  VkPresentModeKHR LveSwapChain::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
    ...
    // prende l'immagine più recente. Vantaggio: latenza di input ridotta e nessun tearing.
    // Svantaggio: consumo energetico elevato poiché la GPU lavora costantemente al 100% (non ideale
    // su dispositivi a batteria).
    ...
    // 3. FIFO (V-Sync standard - fallback garantito da Vulkan):
    // La swap chain è una coda di tipo FIFO. Le immagini vengono presentate in sincronia con il
    // refresh del display. Previene il tearing. Se la GPU produce frame più velocemente del refresh
    // del display, entra in stato di idle.
