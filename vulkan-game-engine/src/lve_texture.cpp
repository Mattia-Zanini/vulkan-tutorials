...
namespace lve {
  // Costruttore principale per texture da file: carica l'immagine da disco, crea l'image view, il sampler e il descrittore
  ...
  void LveTexture::createTextureImage(const std::string& filepath) {
    ...
    // Carica i byte dell'immagine forzando 4 canali per pixel (RGBA)
    stbi_uc* pixels =
    ...
    // Staging buffer allocato in memoria visibile dall'host (CPU) per caricare i pixel iniziali
    ...
    // Rilascia i pixel della CPU dopo la copia nello staging buffer
    ...
    // Alloca l'immagine in memoria ad alte prestazioni locale della GPU (DEVICE_LOCAL)
    ...
    // Transizione 1: prepara l'immagine a ricevere i dati dallo staging buffer
    mDevice.transitionImageLayout(
      ...
      mMipLevels,
      mLayerCount);
    // Copia i pixel dallo staging buffer alla VkImage
    ...
    // Transizione 2: trasforma il layout a SHADER_READ_ONLY_OPTIMAL per consentire il campionamento negli shader
    mDevice.transitionImageLayout(
      mTextureImage,
      VK_FORMAT_R8G8B8A8_SRGB,
      VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
  ...
  // Crea la vista dell'immagine (VkImageView) per consentire agli shader di accedere ai pixel
  ...
  // Configura il campionatore (filtri di interpolazione bilineare, wrapping UV e filtro anisotropico 16x)
