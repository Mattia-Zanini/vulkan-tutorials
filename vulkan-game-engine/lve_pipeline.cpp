...
namespace lve {
        ...
        const PipelineConfigInfo& configInfo)
    {
        ...
        // Collega tutti gli stadi programmabili e le configurazioni a funzione fissa definite in configInfo
    ...
    PipelineConfigInfo LvePipeline::defaultPipelineConfigInfo(uint32_t width, uint32_t height) {
        ...
        // consecutivi forma un triangolo separato. primitiveRestartEnable permette (se true) di spezzare
        // geometrie continue (strip) inserendo un indice speciale nell'index buffer.
        ...
        // VIEWPORT & SCISSOR
        // ==========================================
        // Il Viewport trasforma le coordinate normalizzate di gl_Position ([-1, 1]) nello spazio pixel dell'immagine (width, height).
        // minDepth e maxDepth specificano l'intervallo di profondità per la coordinata Z.
        ...
        // Lo Scissor definisce un rettangolo di ritaglio: i pixel al di fuori vengono scartati invece di essere ridimensionati.
        configInfo.scissor.offset = { 0, 0 };
        configInfo.scissor.extent = { width, height };

        // Combina Viewport e Scissor (alcune GPU supportano array di viewport multipli con apposite feature).
        ...
        // Se abilitato (true), i frammenti oltre i piani di clipping (z < 0 o z > 1) vengono clampati invece che scartati
        configInfo.rasterizationInfo.depthClampEnable = VK_FALSE;
        // Se abilitato (true), scarta tutte le primitive prima del rasterizer (usato se non si vuole output grafico)
        configInfo.rasterizationInfo.rasterizerDiscardEnable = VK_FALSE;
        // FILL disegna i triangoli pieni (altre opzioni: LINE per wireframe, POINT per soli vertici)
        configInfo.rasterizationInfo.polygonMode = VK_POLYGON_MODE_FILL;
        configInfo.rasterizationInfo.lineWidth = 1.0f;
        // Cull mode: scarta le facce dei triangoli in base alla loro direzione apparente (winding order) e punto di vista
        configInfo.rasterizationInfo.cullMode = VK_CULL_MODE_NONE;
        configInfo.rasterizationInfo.frontFace = VK_FRONT_FACE_CLOCKWISE;
        // Depth bias: altera i valori di profondità (usato per tecniche come shadow mapping per evitare shadow acne)
        ...
        // COLOR BLENDING
        // ==========================================
        // Controlla come combinare il colore calcolato dal fragment shader con quello già presente nel framebuffer.
        // Maschera dei canali RGBA che possono essere scritti
        ...
        // Con LESS, il nuovo frammento viene disegnato solo se la sua profondità è minore (più vicino) rispetto a quella nel buffer.
